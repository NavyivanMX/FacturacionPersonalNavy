...
Public Class frmFacturar
    ...
    Private Sub CARGARNOTA(ByVal RES As String, ByVal NOTA As String)
        If Not CHECACONZ() Then
            Exit Sub
        End If
        DGV.DataSource = BDLlenatabla("SELECT D.PLATILLO,D.CANTIDAD,P.NOMBRE DESCRIPCION,PRECIO=(CONVERT(NUMERIC(15,2),(D.TOTAL/D.CANTIDAD)/1.16,2)),TOTAL=CONVERT(NUMERIC(15,2),(D.TOTAL/1.16),2) FROM DETALLENOTASDEVENTA D INNER JOIN PLATILLOS P ON D.PLATILLO=P.CLAVE WHERE D.RESTAURANTE='" + RES + "' AND NOTA=" + NOTA.ToString, CADENA)
    ...
    Private Sub CBEMISOR_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CBEMISOR.SelectedIndexChanged
        ...
        LLENACOMBOBOX2LISTAS(CBNEG, LNEG, LSERIE, "SELECT CLAVE,SERIE,NOMBRE FROM NEGOCIOS WHERE RFC='" + CBEMISOR.Text + "' ORDER BY NOMBRE", frmPrincipal.CadenaConexion)
