...
Public Class frmExcel2PDFXML
    ...
    Private Sub BTNBUSCAR_Click(sender As Object, e As EventArgs) Handles BTNBUSCAR.Click
        CARGAREXCEL()
    End Sub
    Private Sub ObtenerXML(ByVal RFCEMISOR As String, ByVal UUID As String)
        Try
            ' esto es para 4.5
            ...
            Dim xml As New WSNavyUtilerias.get_xml
            xml.username = My.Settings.USUNS
            xml.password = My.Settings.PWDNS
            xml.uuid = UUID
            xml.taxpayer_id = RFCEMISOR
            Dim response As New WSNavyUtilerias.get_xmlResponse
            Dim soap As New WSNavyUtilerias.UtilitiesSOAP

            response = soap.get_xml(xml)


            If response.get_xmlResult.error = "" Then
                Dim url As String
                Dim usuario As String
                usuario = Environment.UserName
                url = "C:\Users\" & usuario & "\Documents\"

                Dim objStreamWriter = New StreamWriter(url & UUID & ".xml")

                Dim x = New XmlSerializer(response.get_xmlResult.xml.GetType())

                x.Serialize(objStreamWriter, response.get_xmlResult.xml)

                objStreamWriter.Close()

            Else
                MsgBox("No se encontro ese comprobante Fiscal")
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    ...
    Private Sub BTNXML_Click(sender As Object, e As EventArgs) Handles BTNXML.Click
        ObtenerXML(DGV.Item(0, DGV.CurrentRow.Index).Value.ToString, DGV.Item(3, DGV.CurrentRow.Index).Value.ToString)
        ...
        For X = 0 To TREG
            ...
            DT = BDLlenatabla(QUERY, CadenaConexionZ)
    ...
    Private Sub GENERAPDF(ByVal TFAC As Integer, ByVal RUTAARCHIVOPDF As String, ByVal RFCEMISOR As String, ByVal SERIE As String, ByVal FOLIO As Integer, ByVal VERSIONCFD As String)
        ...
        If VERSIONCFD = "3.3" Then
            ...
            GUARDARREPORTEDIRECTO(REP, BDLlenatabla(QUERY, CadenaConexionZ), CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, RUTAARCHIVOPDF, "Enviar a OneNote 2007")
        Else
