...
Public Class frmEmisor
    ...
    Private Sub INICIALIZAR()
        OPLlenaComboBox(CBRFC, LEMI, "SELECT RFC,RFC EMISOR FROM EMISORES WHERE EMPRESA=" + frmPrincipal.CORPORATIVO.ToString + " ORDER BY RFC", frmPrincipal.CadenaConexion)
    ...
    Private Sub GUARDAR()
        ...
        SQLCER.Parameters.Add("@RPFX", SqlDbType.VarChar).Value = TXTCPFX.Text
    ...
    Private Sub CBRFC_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CBRFC.SelectedIndexChanged
        If CBRFC.Text.Length = 13 Then
            OPLlenaComboBox(TXTRF, LRF, "SELECT CLAVE,NOMBRE FROM CSATREGIMENFISCAL WHERE ACTIVO=1 AND FISICA=1 ORDER BY NOMBRE", frmPrincipal.CadenaConexion)
        Else
            OPLlenaComboBox(TXTRF, LRF, "SELECT CLAVE,NOMBRE FROM CSATREGIMENFISCAL WHERE ACTIVO=1 AND MORAL=1 ORDER BY NOMBRE", frmPrincipal.CadenaConexion)
        End If
