Public Class frmNegocios
    ...
    Private Sub frmNegocios_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            OPLlenaCombobox(CBEMISOR, LDE, "SELECT NOMBRE,RFC FROM EMISORES WHERE EMPRESA=" + frmPrincipal.CORPORATIVO.ToString + "", frmPrincipal.CadenaConexion)
            OPLlenaCombobox(txtPais, LPAIS, "SELECT CLAVE,NOMBRE FROM PAISES WHERE ACTIVO=1 ORDER BY NOMBRE", frmPrincipal.CadenaConexion)
            OPCargaX(Me.LPAIS, CBEMISOR, "MEX")
    ...
    Private Sub CBEDO_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CBEDO.SelectedIndexChanged
        If frmPrincipal.CHECACONX Then
            OPLlenaCombobox(txtMunicipio, LMUN, ("SELECT CLAVE,NOMBRE FROM MUNICIPIOS WHERE ESTADO='" & LEDO(CBEDO.SelectedIndex) & "' AND ACTIVO=1 ORDER BY NOMBRE"), frmPrincipal.CadenaConexion)
            OPLlenaCombobox(txtLocalidad, LLOCA, ("SELECT CLAVE,NOMBRE FROM LOCALIDADES WHERE ESTADO='" & LEDO(CBEDO.SelectedIndex) & "' AND ACTIVO=1 ORDER BY NOMBRE"), frmPrincipal.CadenaConexion)
        End If
    End Sub
    ...
    Private Sub txtPais_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtPais.SelectedIndexChanged
        Try
            If frmPrincipal.CHECACONX Then
                OPLlenaCombobox(CBEDO, LEDO, ("SELECT CLAVE,NOMBRE FROM ESTADOS WHERE PAIS='" & LPAIS(txtPais.SelectedIndex) & "' AND ACTIVO=1 ORDER BY NOMBRE"), frmPrincipal.CadenaConexion)
            End If
        Catch ex As Exception
